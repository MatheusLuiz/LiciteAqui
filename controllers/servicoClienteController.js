...
class ServicoClienteController {
  static async cadastrar(req, res) {
    try {
      ...
      return res
        .status(201)
        .json({
          message: "Serviço de cliente cadastrado com sucesso!",
          result,
        });
    } catch (error) {
      console.error(error);
      return res
        .status(500)
        .json({ error: "Erro ao cadastrar o serviço de cliente." });
    }
  }
  ...
  static async atualizar(req, res) {
    try {
      ...
      const result = await ServicoClienteModel.atualizarServicoCliente(
        id_servico,
        id_cliente,
        id_tipo_servico,
        usuario
      ...
      return res
        .status(500)
        .json({ error: "Erro ao atualizar o serviço de cliente." });
    }
  }
  ...
  static async listar(req, res) {
    try {
      ...
      if (!servicosCliente || servicosCliente.length === 0) {
        return res
          .status(404)
          .json({ message: "Nenhum serviço de cliente encontrado." });
      }
      ...
      return res
        .status(500)
        .json({ error: "Erro ao listar os serviços de cliente." });
    }
  }
  ...
  static async deletar(req, res) {
    try {
      ...
      const result = await ServicoClienteModel.deletarServicoCliente(
        id_servico,
        usuario
      );
      return res
        .status(200)
        .json({ message: "Serviço de cliente deletado com sucesso!", result });
    } catch (error) {
      console.error(error);
      return res
        .status(500)
        .json({ error: "Erro ao deletar o serviço de cliente." });
    }
  }
}
